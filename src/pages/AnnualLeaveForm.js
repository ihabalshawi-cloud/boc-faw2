...
function AnnualLeaveForm({ emp }) {
  ...
  const saveAndSubmit = async () => {
    ...
    const today = new Date().toISOString().split("T")[0];
    storage.set(STORAGE_KEY, { name, jobNum, jobTitle, dept, fromDate, toDate, days, purpose, reqDate, sigDataUrl, status: "submitted", submittedDate: today });
    setStatus("submitted");
...
<div class="disclaimer">يعتبر هذا النموذج ملك لشركة نفط البصرة فقط، لايجوز نسخه او الكشف عن محتواه بدون موافقة خطية مسبقة من قبل شركة نفط البصرة.</div>
  ...
  const printForm = () => {
    ...
    const finalHtml = buildAnnualHtml().replace("</body>", toolbar + "</body>");
