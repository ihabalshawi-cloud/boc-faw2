...
export function TsExportPanel({ gDrive, exporting, exportDriveId, setExportDriveId, exportFromFile, exportFromDrive, exportFromBuiltin, exportFileRef }) {
  return (
    <div className="card rounded-xl p-4 border border-green-200 bg-green-50 space-y-3" dir="rtl">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex-1 min-w-[200px]">
          <p className="text-xs font-semibold text-green-700 mb-1">قالب الملاك (مدمج):</p>
          <button onClick={exportFromBuiltin} disabled={exporting}
            ...
            <FileCheck size={14}/> {exporting ? "جارٍ التصدير..." : "تصدير نموذج الملاك"}
          </button>
        </div>
        <div className="flex-1 min-w-[200px]">
          ...
          <button onClick={()=>exportFileRef.current?.click()} disabled={exporting}
            ...
            <Upload size={14}/> {exporting ? "جارٍ التصدير..." : "اختر قالب Excel"}
          </button>
        </div>
        {gDrive?.isReady && (
          <div className="flex-1 min-w-[240px]">
