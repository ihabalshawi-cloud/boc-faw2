...
export const FirebaseAPI = {
  ...
  loadEmpViews: async (empId) => { try { const r=await fetch(`${FIREBASE_URL}/emp_views/${empId}.json`); if(!r.ok)return null; const d=await r.json(); if(d===null)return[]; if(Array.isArray(d))return d; if(d&&typeof d==="object")return Object.values(d).filter(v=>typeof v==="string"); return[]; } catch{return null;} },
