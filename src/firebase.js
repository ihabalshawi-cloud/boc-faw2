...
export const FirebaseAPI = {
  ...
  saveEmpViews: async (empId, views) => { try { return (await fetch(`${FIREBASE_URL}/emp_views/${empId}.json`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(Array.isArray(views)?views:[])})).ok; } catch{return false;} },
  loadEmpViews: async (empId) => { try { const r=await fetch(`${FIREBASE_URL}/emp_views/${empId}.json`); if(!r.ok)return null; const d=await r.json(); if(d===null)return null; if(Array.isArray(d))return d; if(d&&typeof d==="object")return Object.values(d).filter(v=>typeof v==="string"); return []; } catch{return null;} },
  loadAllEmpViews: async () => { try { const r=await fetch(`${FIREBASE_URL}/emp_views.json`); if(!r.ok)return null; const d=await r.json(); if(!d||typeof d!=="object"||Array.isArray(d))return{}; const out={};Object.entries(d).forEach(([k,v])=>{out[k]=Array.isArray(v)?v:(v&&typeof v==="object"?Object.values(v).filter(x=>typeof x==="string"):[]);});return out; } catch{return null;} },
