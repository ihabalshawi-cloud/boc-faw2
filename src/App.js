...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    if (isValid) {
      ...
      if (isConnected) await FirebaseAPI.saveLoginHistory(loginEntry);
...
function RequestsPage({ emp }) {
  ...
  const handleSubmit = async () => {
    ...
    if (isConnected) await FirebaseAPI.saveRequest(newRequest);
    
    const updated = [newRequest, ...requests];
    setRequests(updated);
    storage.set(`requests_${emp.id}`, updated);
    
    // إضافة إلى جميع الطلبات
...
function ApprovalsPage({ emp }) {
  ...
  const updateStatus = async (id, status) => {
    ...
    if (isConnected) await FirebaseAPI.saveNotification(request.empId, notification);
...
function NotificationsPage({ emp }) {
  ...
  const markAsRead = async (id) => {
    ...
    if (notif && !notif.read) {
      ...
      if (isConnected) await FirebaseAPI.saveNotification(emp.id, updated);
...
function AttendanceSystem({ emp, isAdmin, allEmployees }) {
  ...
  const handleCheckIn = async () => {
    ...
    if (isConnected) await FirebaseAPI.saveAttendance(emp.id, selectedDate, updated);
...
function TrainingSystem({ emp, isAdmin, allEmployees }) {
  ...
  const addTraining = async () => {
    ...
    if (isConnected) await FirebaseAPI.saveTraining(emp.id, newTraining);
  ...
  const updateStatus = async (id, status) => {
    ...
    if (training && isConnected) await FirebaseAPI.saveTraining(emp.id, { ...training, status });
    storage.set(`trainings_${emp.id}`, updated);
