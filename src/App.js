  ...
  return (
    <div className="space-y-4">
      ...
      <div id="print-inventory" className="bg-white rounded-2xl border overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-right text-xs"><thead><tr className="bg-slate-50 border-b"><th>الاسم</th><th>الرمز</th><th>الفئة</th><th>الكمية</th><th>حد التنبيه</th><th>الحالة</th><th>الموقع</th>{canEdit && <th>إجراءات</th>}</table></thead>
        <tbody>{filtered.map(it=>{
          ...
          return (<tr key={it.id} className={`border-b hover:bg-slate-50 ${isLow ? "bg-amber-50" : ""}`}>
            ...
            {canEdit && <td className="px-3 py-2"><div className="flex gap-1"><button onClick={()=>openEdit(it)} className="p-1 text-blue-500"><Edit3 size={12}/></button><button onClick={()=>deleteItem(it.id)} className="p-1 text-red-400"><Trash2 size={12}/></button></div></td>}</td>);
        })}</tbody><td></div>
      </div>
      ...
      <div className="bg-white rounded-2xl border overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50">
            <tr><th className="p-3 text-right">الاسم</th><th>الرقم</th><th>المسمى</th><th>القسم</th><th>الدوام</th><th>الصلاحية</th>{canDelete && <th></th>}</tr>
          </thead>
          <tbody>
            {filtered.map(e => (
              <tr key={e.id} className="border-t">
                ...
                {canDelete && <td className="p-3"><button onClick={()=>deleteEmp(e.id)} className="text-red-400"><Trash2 size={14}/></button></td>}
              </tr>
            ))}
          </tbody>
        </table>
