...
/* ═══════════════════════════════════════════════════════════
   ACCOUNTS - الكادر الكامل (30 موظف)
═══════════════════════════════════════════════════════════ */
const ACCOUNTS = [
...
/* ═══════════════════════════════════════════════════════════
   FIREBASE API - للمزامنة بين الأجهزة
═══════════════════════════════════════════════════════════ */
const FirebaseAPI = {
  // التحقق من الاتصال
  ...
  saveAttendance: async (empId, date, record) => {
    try {
      await fetch(`${FIREBASE_URL}/attendance/${empId}/${date}.json`, { 
        method: "PUT", 
        headers: { "Content-Type": "application/json" },
  ...
  getAttendance: async (empId) => {
    try {
      const res = await fetch(`${FIREBASE_URL}/attendance/${empId}.json`);
      if (!res.ok) return {};
      const data = await res.json();
      return data || {};
    } catch { return {}; }
  ...
  saveTraining: async (empId, training) => {
    try {
      await fetch(`${FIREBASE_URL}/training/${empId}/${training.id}.json`, { 
        method: "PUT", 
        headers: { "Content-Type": "application/json" },
  ...
  getTrainings: async (empId) => {
    try {
      const res = await fetch(`${FIREBASE_URL}/training/${empId}.json`);
      if (!res.ok) return [];
      const data = await res.json();
      return data ? Object.values(data) : [];
    } catch { return []; }
  ...
  // ========== مزامنة شاملة ==========
  syncAllData: async (empId, localData) => {
    // مزامنة الطلبات
    const requests = storage.get(`requests_${empId}`, []);
    for (const req of requests) {
      await FirebaseAPI.saveRequest(req);
    }
    // مزامنة الحضور
    const attendance = storage.get(`attendance_${empId}`, {});
    for (const [date, record] of Object.entries(attendance)) {
      await FirebaseAPI.saveAttendance(empId, date, record);
    }
    // مزامنة التدريب
    const trainings = storage.get(`trainings_${empId}`, []);
    for (const training of trainings) {
      await FirebaseAPI.saveTraining(empId, training);
    }
    return true;
  }
};
...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    // 1. التحقق المباشر من كلمة المرور الافتراضية
    if (pass.trim() === account.password) {
      isValid = true;
    }
    ...
    // 3. التحقق من Firebase (إذا كان متصلاً)
    ...
    if (isValid) {
      // حفظ كلمة المرور في Firebase إذا كانت افتراضية
      if (isConnected && pass.trim() === account.password) {
        ...
        setSyncStatus("✓ تمت المزامنة مع السحابة");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center p-4" dir="rtl">
      <div className="w-full max-w-md bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-white/20">
        ...
        <div className="mb-4 flex items-center justify-center gap-2 text-xs">
          {isConnected ? (
            <><Wifi size={12} className="text-emerald-400"/><span className="text-emerald-400">✓ متصل بالسحابة - مزامنة تلقائية</span></>
          ) : (
            <><WifiOff size={12} className="text-amber-400"/><span className="text-amber-400">⚠ غير متصل - يعمل محلياً</span></>
          )}
        </div>
        {syncStatus && <div className="mb-4 text-center text-xs text-emerald-400">{syncStatus}</div>}
...
function ChangePasswordPage({ emp, onLogout }) {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (isConnected) {
        await FirebaseAPI.savePassword(emp.id, newPass.trim());
        setMsg({ text: "✅ تم تغيير كلمة المرور ومزامنتها مع السحابة!", type: "success" });
      } else {
        setMsg({ text: "✅ تم تغيير كلمة المرور محلياً (غير متصل)", type: "success" });
      }
  ...
  useEffect(() => {
    const loadRequests = async () => {
      ...
      if (isConnected) {
        ...
        storage.set(`requests_${emp.id}`, myRequests);
      } else {
...
function ApprovalsPage({ emp }) {
  ...
  const updateStatus = async (id, status) => {
    ...
    const updated = { ...request, status, decidedAt: new Date().toISOString(), decidedBy: emp.name };
    ...
    // تحديث محلياً
    const allRequests = storage.get("all_requests", []);
    const updatedAll = allRequests.map(r => r.id === id ? updated : r);
    storage.set("all_requests", updatedAll);
    
    // تحديث طلبات الموظف
    const empRequests = storage.get(`requests_${request.empId}`, []);
    const updatedEmp = empRequests.map(r => r.id === id ? updated : r);
    storage.set(`requests_${request.empId}`, updatedEmp);
    ...
    if (isConnected) {
      await FirebaseAPI.saveNotification(request.empId, notification);
    }
    const empNotifs = storage.get(`notifications_${request.empId}`, []);
    storage.set(`notifications_${request.empId}`, [notification, ...empNotifs]);
...
function NotificationsPage({ emp }) {
  ...
  useEffect(() => {
    const loadNotifications = async () => {
      ...
      if (isConnected) {
        ...
        storage.set(`notifications_${emp.id}`, notifs);
      } else {
  ...
  const markAsRead = async (id) => {
    ...
    if (notif && !notif.read) {
      ...
      const updatedNotifs = notifications.map(n => n.id === id ? updated : n);
      setNotifications(updatedNotifs);
      storage.set(`notifications_${emp.id}`, updatedNotifs);
    }
  };
...
function AttendanceSystem({ emp, isAdmin, allEmployees }) {
  ...
  useEffect(() => {
    const loadAttendance = async () => {
      setLoading(true);
      const targetId = selEmpId || emp.id;
      if (isConnected) {
        const fbData = await FirebaseAPI.getAttendance(targetId);
        setDailyRecords(fbData);
        storage.set(`attendance_${targetId}`, fbData);
      } else {
        const local = storage.get(`attendance_${targetId}`, {});
        setDailyRecords(local);
      }
      setLoading(false);
    };
    loadAttendance();
  }, [selEmpId, emp.id, isConnected]);
  
  const handleCheckIn = async () => {
    const nowTime = new Date().toLocaleTimeString("ar-IQ", { hour:"2-digit", minute:"2-digit" });
    const record = dailyRecords[selectedDate] || {};
    const updated = { ...record, checkIn: nowTime, status: "حاضر" };
    const newRecords = { ...dailyRecords, [selectedDate]: updated };
    setDailyRecords(newRecords);
    
    if (isConnected) {
      await FirebaseAPI.saveAttendance(emp.id, selectedDate, updated);
    }
    storage.set(`attendance_${emp.id}`, newRecords);
...
function TrainingSystem({ emp, isAdmin, allEmployees }) {
  ...
  useEffect(() => {
    const loadTrainings = async () => {
      setLoading(true);
      if (isConnected) {
        const fbData = await FirebaseAPI.getTrainings(emp.id);
        setTrainings(fbData);
        storage.set(`trainings_${emp.id}`, fbData);
      } else {
        const local = storage.get(`trainings_${emp.id}`, []);
        setTrainings(local);
      }
      setLoading(false);
    };
  ...
  const addTraining = async () => {
    ...
    if (isConnected) {
      await FirebaseAPI.saveTraining(emp.id, newTraining);
    }
  ...
  const updateStatus = async (id, status) => {
    const updated = trainings.map(t => t.id === id ? { ...t, status, completedAt: status==="مكتملة"?new Date().toISOString():t.completedAt } : t);
    setTrainings(updated);
    
    const training = trainings.find(t => t.id === id);
    if (training && isConnected) {
      await FirebaseAPI.saveTraining(emp.id, { ...training, status });
    }
    storage.set(`trainings_${emp.id}`, updated);
...
function SyncButton({ onSync, isSyncing }) {
  return (<button onClick={onSync} disabled={isSyncing} className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 rounded-xl text-xs font-bold transition-all disabled:opacity-50"><RefreshCw size={12} className={isSyncing ? "animate-spin" : ""}/> {isSyncing ? "جاري المزامنة..." : "مزامنة مع السحابة"}</button>);
}
...
function Dashboard({ emp, onLogout }) {
  ...
  const handleSync = async () => {
    if (!isConnected) { alert("لا يوجد اتصال بالسحابة"); return; }
    setIsSyncing(true);
    await FirebaseAPI.syncAllData(emp.id);
    // إعادة تحميل البيانات
    const freshRequests = await FirebaseAPI.getAllRequests();
    setAllRequests(freshRequests);
    storage.set("all_requests", freshRequests);
    setIsSyncing(false);
  ...
  return (
    <div className="min-h-screen bg-slate-50" dir="rtl">
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-xs">
            {isConnected ? <><Wifi size={12} className="text-emerald-500"/><span className="text-emerald-700">متصل بالسحابة</span></> : <><WifiOff size={12} className="text-amber-500"/><span className="text-amber-700">غير متصل</span></>}
          </div>
          {isAdmin && isConnected && <SyncButton onSync={handleSync} isSyncing={isSyncing} />}
          <div className="text-left"><p className="text-sm font-bold">{emp.name.split(" ").slice(0,2).join(" ")}</p><p className="text-xs text-slate-500">{emp.title}</p></div>
      ...
      <div className="mt-6 p-3 bg-slate-50 rounded-xl text-xs"><p className="font-bold mb-2">ℹ️ النظام</p><p>✓ مزامنة سحابية</p><p>✓ يعمل دون اتصال</p>{isConnected && <p className="text-[10px] text-emerald-600 mt-1">✓ متزامن مع السحابة</p>}</div></aside>
      ...
      <main className="flex-1 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6"><div className="bg-emerald-50 border rounded-xl p-4"><CheckCircle className="text-emerald-600"/><p className="font-bold mt-2">✓ النظام يعمل</p>{isConnected && <p className="text-xs text-emerald-600 mt-1">✓ متصل بالسحابة</p>}</div><div className="bg-blue-50 border rounded-xl p-4"><Shield className="text-blue-600"/><p className="font-bold mt-2">✓ آمن ومشفر</p><p className="text-xs text-blue-600 mt-1">بياناتك متزامنة</p></div></div>
        <div className="mt-6 pt-6 border-t"><p className="text-sm text-slate-600">الوحدات المتاحة: طلبات الإجازة | الحضور | التدريب | الجرد | الإشعارات | التقارير</p></div></div>}
