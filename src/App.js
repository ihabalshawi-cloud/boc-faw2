...
const ROLES = {
  SUPER_ADMIN: { level: 5, name: "مشرف عام", permissions: { employees: { view: true, add: true, edit: true, delete: true, assignRole: true }, requests: { view: true, approve: true, reject: true }, inventory: { view: true, add: true, edit: true, delete: true }, attendance: { view: true, edit: true }, training: { view: true, assign: true }, reports: { view: true, export: true } } },
  ADMIN: { level: 4, name: "مدير إداري", permissions: { employees: { view: true }, requests: { view: true, approve: true, reject: true }, inventory: { view: true }, attendance: { view: true, edit: true }, training: { view: true }, reports: { view: true, export: true } } },
  ...
  TRAINING_COORDINATOR: { level: 3, name: "منسق تدريب", permissions: { training: { view: true, assign: true, evaluate: true }, reports: { view: true, export: true } } },
  ATTENDANCE_ADMIN: { level: 2, name: "مشرف حضور", permissions: { attendance: { view: true, edit: true, export: true }, reports: { view: true, export: true } } },
  EMPLOYEE: { level: 1, name: "موظف", permissions: { requests: { view: true, add: true }, attendance: { view: true }, training: { view: true }, reports: { view: true } } }
};
...
const INITIAL_INVENTORY = [
  ...
  { id:3, code:"5869856050", name:"NEEDLE SOLENOID", category:"صمامات", qty:2, minAlert:1, condition:"جيد", location:"مخزن الآلات الدقيقة" },
  { id:4, code:"", name:"كيج 60 psi", category:"مقاييس ضغط", qty:25, minAlert:5, condition:"جيد", location:"مخزن الآلات الدقيقة" },
  { id:5, code:"", name:"عكس حرف T", category:"قطع توصيل", qty:150, minAlert:20, condition:"جيد", location:"مخزن الآلات الدقيقة" },
];
          ...
          <div key={r.id} className="bg-white rounded-2xl p-4 border">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="mt-2 text-sm">{r.dateFrom} إلى {r.dateTo} — {r.days} يوم</p>
              ...
              {r.status === "بانتظار المراجعة" && (
                <button onClick={()=>deleteReq(r.id)} className="p-2 text-red-400"><Trash2 size={18}/></button>
              )}
            </div>
          </div>
        ))
    ...
    <div className="space-y-4">
      ...
      {(adding || editId) && (canAdd || canEdit) && (
        <div className="bg-white rounded-2xl border-2 border-blue-200 p-5">
          ...
          <div className="flex gap-2 justify-end mt-4"><button onClick={()=>{setEditId(null);setAdding(false);}} className="px-4 py-2 text-sm font-semibold text-slate-600 bg-slate-100 rounded-xl">إلغاء</button><button onClick={saveItem} className="px-4 py-2 text-sm font-bold text-white bg-blue-600 rounded-xl"><Save size={13}/> حفظ</button></div>
        </div>
      )}
      ...
      <div id="print-inventory" className="bg-white rounded-2xl border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-right text-xs">
            <thead><tr className="bg-slate-50 border-b"><th>الاسم</th><th>الرمز</th><th>الفئة</th><th>الكمية</th><th>حد التنبيه</th><th>الحالة</th><th>الموقع</th>{canEdit && <th>إجراءات</th>}</tr></thead>
            <tbody>{filtered.map(it=>{
              const isLow = it.qty <= it.minAlert;
              return (
                <tr key={it.id} className={`border-b hover:bg-slate-50 ${isLow ? "bg-amber-50" : ""}`}>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-1">
                      {canEdit && <button onClick={()=>updateQuantity(it.id, Math.max(0, it.qty-1))} className="px-1.5 py-0.5 bg-red-100 text-red-600 rounded">-</button>}
                      <span className={`font-bold w-8 text-center ${isLow ? "text-amber-600" : "text-slate-800"}`}>{it.qty}</span>
                      {canEdit && <button onClick={()=>updateQuantity(it.id, it.qty+1)} className="px-1.5 py-0.5 bg-emerald-100 text-emerald-600 rounded">+</button>}
                    </div>
                  </td>
                  ...
                  {canEdit && <td className="px-3 py-2"><div className="flex gap-1"><button onClick={()=>openEdit(it)} className="p-1 text-blue-500"><Edit3 size={12}/></button><button onClick={()=>deleteItem(it.id)} className="p-1 text-red-400"><Trash2 size={12}/></button></div></td>}
                </tr>
              );
            })}</tbody>
          </table>
      ...
      <div className="bg-white rounded-2xl border overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50">
            <tr><th className="p-3 text-right">الاسم</th><th>الرقم</th><th>المسمى</th><th>القسم</th><th>الدوام</th><th>الصلاحية</th></tr>
          </thead>
          <tbody>
            {filtered.map(e => (
              <tr key={e.id} className="border-t">
                <td className="p-3">{e.name}</td>
                <td className="p-3">{e.jobNum}</td>
                <td className="p-3">{e.title}</td>
                <td className="p-3">{e.dept}</td>
                <td className="p-3">{e.shift}{e.group?` [${e.group}]`:""}</td>
...
function Dashboard({ emp, onLogout }) {
  ...
  if (rolePermissions.requests?.approve) {
    menuItems.unshift({ id: "approvals", label: "الموافقات", icon: <ThumbsUp size={18}/> });
  }
  if (rolePermissions.inventory?.view) {
    menuItems.push({ id: "inventory", label: "المخزن", icon: <Package size={18}/> });
  }
  if (rolePermissions.employees?.view) {
    menuItems.push({ id: "employees", label: "الموظفين", icon: <Users size={18}/> });
  }
      ...
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-left">
            ...
            <div className="flex items-center gap-1"><Shield size={10} className="text-slate-400"/><p className="text-xs text-slate-500">{rolePermissions.name}</p></div>
          </div>
          ...
          {view === "home" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border">
                  <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><Users size={18}/> توزيع الصلاحيات</h3>
                  <div className="space-y-3">
                    {Object.entries({
