...
const PERMISSIONS_DEF = {
  ...
  EXPORT_LEAVE_EXCEL: { label:"تصدير نماذج الإجازة",   icon:"📤" },
...
const BUILT_IN_ROLES = {
  ...
  ATTENDANCE_ADMIN:  { label:"مدير حضور",     color:"bg-violet-100 text-violet-800", permissions:["EXPORT_LEAVE_EXCEL","VIEW_LOGIN_HIST"] },
...
function hasPermission(emp, perm) {
  ...
  const roleName = s.role || (emp.role === "admin" ? "SUPER_ADMIN" : emp.role === "inventory_manager" ? "WAREHOUSE_MANAGER" : emp.role === "attendance_admin" ? "ATTENDANCE_ADMIN" : "EMPLOYEE");
